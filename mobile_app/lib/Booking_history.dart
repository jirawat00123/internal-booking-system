...
class _BookingHistoryScreenState extends State<BookingHistoryScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text(
          'ประวัติและสถานะ',
          style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 18, fontFamily: 'Kanit'),
        ),
        centerTitle: true,
      ),
      body: Column(
            ...
            child: isLoading
                ...
                : ValueListenableBuilder<List<BookingHistory>>(
                    ...
                    builder: (context, historyList, child) {
                      ...
                      final todayMidnight = DateTime(now.year, now.month, now.day); // 💡 แก้จาก date.day เป็น now.day

                      // 1. กรองรายการที่ 'เสร็จสิ้น' หรือ 'ยกเลิกแล้ว' ของวันเก่าออกไป (หลังผ่านเที่ยงคืน)
                      List<BookingHistory> validTimeList = historyList.where((item) {
                        try {
                          ...
                          if ((item.currentStatus == 'เสร็จสิ้น' || item.currentStatus == 'ยกเลิกแล้ว') && 
                      ...
                      if (selectedTab == 'ทั้งหมด') {
                        ...
                        filteredList = validTimeList.where((item) => item.type == selectedTab).toList();
                      }
            ...
            children: [
              ClipRRect(
                ...
                child: Container(
                  ...
                  child: Icon(
                    booking.type == 'ห้องประชุม' ? Icons.meeting_room : Icons.directions_car,
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      booking.type == 'ห้องประชุม' ? 'Meeting Room ${booking.roomId}' : booking.title,
                      style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF1E293B), fontFamily: 'Kanit'),
                    ),
                    ...
                    Row(
                      children: [
                        ...
                        Text(
                          booking.date,
                          style: const TextStyle(fontSize: 12, color: Colors.grey, fontFamily: 'Kanit'),
                        ),
                      ],
                    ),
                  ],
              ...
              Container(
                ...
                child: Text(
                  statusText,
                  style: TextStyle(color: statusTextColor, fontSize: 10, fontWeight: FontWeight.bold, fontFamily: 'Kanit'),
                ),
              ),
            ],
          ),
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                const Text('เวลาการจอง', style: TextStyle(fontSize: 12, color: Colors.blueGrey, fontFamily: 'Kanit')),
                const Spacer(),
                Text(
                  '${_formatTime(booking.startTime)} - ${_formatTime(booking.endTime)} น.',
                  style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Color(0xFF004AAD), fontFamily: 'Kanit'),
                ),
              ],
            ),
          ),
                ...
                children: [
                  Expanded(
                    child: OutlinedButton(
                      ...
                      child: const Text('ดูรายละเอียด', style: TextStyle(color: Color(0xFF1E293B), fontWeight: FontWeight.bold, fontSize: 13, fontFamily: 'Kanit')),
                    ),
                  ),
                      ...
                      child: OutlinedButton(
                        onPressed: () {
                          ...
                          setState(() {
                            ...
                            globalBookingHistory.value = List<BookingHistory>.from(globalBookingHistory.value);
                          });
              ...
              if (statusText == 'กำลังใช้งาน') ...[
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    child: const Text('คืนห้องก่อนเวลา', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 14, fontFamily: 'Kanit')),
                  ),
                ),
              ],
            ],
            ...
            child: Column(
              ...
              children: [
                const Center(
                  child: Text(
                    'รายละเอียดประวัติ',
                    style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.black87, fontFamily: 'Kanit'),
                  ),
                ),
                ...
                Row(
                  children: [
                    Container(
                      ...
                      child: const Icon(Icons.groups_outlined, color: Color(0xFF004AAD), size: 28),
                    ),
                    const SizedBox(width: 14),
                    Text(
                      booking.type == 'ห้องประชุม' ? 'Meeting Room ${booking.roomId}' : booking.title,
                      style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.black87, fontFamily: 'Kanit'),
                    ),
                  ],
                ),
                const SizedBox(height: 12),
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      _buildPopupRow('สถานะ', booking.currentStatus, isStatus: true),
                      const SizedBox(height: 10),
                      _buildPopupRow('วันที่', booking.date),
                      const SizedBox(height: 10),
                      _buildPopupRow('เวลา', '${_formatTime(booking.startTime)} - ${_formatTime(booking.endTime)}'),
                      const SizedBox(height: 10),
                      _buildPopupRow('ผู้ทำรายการ', booking.bookedBy), 
                      const SizedBox(height: 10),
                      _buildPopupRow('จำนวนคน', '${booking.participantCount} คน'),
                    ],
                  ),
                ),
                const SizedBox(height: 16),
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      const Text(
                        'ข้อมูลเพิ่มเติม',
                        style: TextStyle(fontSize: 12, color: Colors.grey, fontWeight: FontWeight.bold, fontFamily: 'Kanit'),
                      ),
                      const SizedBox(height: 10),
                      _buildPopupRow('หัวข้อประชุม', booking.type == 'ห้องประชุม' ? booking.title : '-'),
                      const SizedBox(height: 10),
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    child: const Text(
                      'ปิดหน้าต่าง',
                      style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold, fontFamily: 'Kanit'),
                    ),
                  ),
                ),
              ],
  ...
  Widget _buildPopupRow(String label, String value, {bool isStatus = false}) {
    ...
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        Text(label, style: const TextStyle(fontSize: 13, color: Colors.grey, fontFamily: 'Kanit')),
        isStatus
            ? Container(
                padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                ...
                child: Text(
                  value, 
                  style: TextStyle(color: textStatusColor, fontSize: 10, fontWeight: FontWeight.bold, fontFamily: 'Kanit'),
                ),
              )
            : Text(value, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Colors.black87, fontFamily: 'Kanit')),
      ],
    );
  }
}