...
async function main() {
  ...
  // 0. ล้างข้อมูลเก่าตามลำดับความสัมพันธ์ (ลบตัวลูกที่มี Foreign Key ก่อนตัวแม่เสมอ)
  await prisma.vehicleLog.deleteMany();
  await prisma.attachment.deleteMany();
  await prisma.auditLog.deleteMany();     // <--- [จุดแก้ไข] เพิ่มการล้างข้อมูลตาราง Audit Log เพื่อป้องกันการติด FK Constraint
  await prisma.vehicleBooking.deleteMany();
  await prisma.roomBooking.deleteMany();
  await prisma.vehicle.deleteMany();
  await prisma.room.deleteMany();
  await prisma.user.deleteMany();       
  await prisma.role.deleteMany();       // <--- [จุดแก้ไข] ล้างข้อมูลตารางสิทธิ์ (ต้องลบหลัง User เสมอ)
  ...
  // 1.5 สร้างสิทธิ์การใช้งาน (Roles) ตั้งต้นให้ตรงกับประเภทข้อมูลของพนักงาน
  const roleNames = ['ADMIN', 'USER', 'GUARD'];
  const roleMap = {};
  for (const name of roleNames) {
    const role = await prisma.role.create({ data: { name } });
    roleMap[name] = role.id; // เก็บ ID ที่ได้จากฐานข้อมูลเพื่อเอาไปแมปต่อ
  }
  ...
  const employeesData = [
    ...
    { code: 'MC-EN0546', name: 'นายภาคภูมิ ชัยรัมย์', posName: 'วิศวกรไฟฟ้า', role: 'USER' },
    { code: 'MC-EN0547', name: 'ว่าที่รต. อุดมศักดิ์ อรัญโสตร', posName: 'วิศวกรไฟฟ้า', role: 'USER' },

    { code: 'MC-EN0240', name: 'นายทรงวิทย์ ขุนจบเมือง', posName: 'ผจก.ฝ่ายวิศวกรรม', role: 'USER' },
    { code: 'MC-EN0472', name: 'นายอัจฉริยะ นวลล่อง', posName: 'วิศวกรการตลาด', role: 'USER' },
    { code: 'MC-SM0610', name: 'นายบันลือโลก วงทะนี', posName: 'วิศวกรขายและการตลาด', role: 'USER' },

    { code: 'MC-WK0635', name: 'นายศิรศักดิ์ เผือกสงค์', posName: 'เจ้าหน้าที่เขียนแบบ', role: 'USER' },

    { code: 'MC-MK0054', name: 'นายกฤฐิพงษ์ ศรีศิริ', posName: 'ผจก.ฝ่ายวิศวกรรมการขายและการตลาด', role: 'USER' },
    { code: 'MC-WK0535', name: 'นางสาวศิวรักษ์ อินทรแหยม', posName: 'ผู้ช่วยวิศวกรการขายและการตลาด', role: 'USER' },
    { code: 'MC-SM0597', name: 'นายวีระชัย ถิ่นฐานทรัพย์', posName: 'Senior Sales and Marketing Manager', role: 'USER' },

    { code: 'MC-EN0320', name: 'นายวุฒิพงษ์ มาตตี', posName: 'หัวหน้าทีมวิศวกรรมติดตั้งเครื่องจักร', role: 'USER' },
    { code: 'MC-EN0373', name: 'นายศับนท์สฤษฏิ์ พรหมฉิม', posName: 'วิศวกรติดตั้งเครื่องจักร', role: 'USER' },

    { code: 'MC-PC0241', name: 'นายสุชาติ สมโพธิ์', posName: 'หัวหน้าแผนกวิศวกรรมซ่อมบำรุง', role: 'USER' },
    { code: 'MC-EN0448', name: 'นายชนสิษฎ์ มิ่งขวัญ', posName: 'วิศวกรซ่อมบำรุง', role: 'USER' },
    { code: 'MC-EN0539', name: 'นายนครินทร์ บุญมาก', posName: 'วิศวกรซ่อมบำรุง', role: 'USER' },
    { code: 'MC-WK0619', name: 'นายวันชนะ สังข์ลักษณ์', posName: 'วิศวกรซ่อมบำรุง', role: 'USER' },

    { code: 'MC-EN0326', name: 'นายสมบูรณ์ กมลภูจิตรคุปต์', posName: 'หัวหน้าทีมวิศวกรสารสนเทศ IT', role: 'USER' },
    { code: 'MC-EN0494', name: 'นายพงศ์พัทธ์ แตงอ่อน', posName: 'วิศวกรสารสนเทศ IT', role: 'USER' },
    { code: 'MC-EN0596', name: 'นายกิตติ บุญเลิศ', posName: 'วิศวกรสารสนเทศ IT', role: 'USER' },
    { code: 'MC-EN0644', name: 'นายรณกฤต เหลืองอ่อน', posName: 'วิศวกรสารสนเทศ IT', role: 'USER' },
    { code: 'MC-EN0646', name: 'นายนิติภูมิ กองฟู', posName: 'วิศวกรสารสนเทศ IT', role: 'USER' },

    { code: 'MC-EN0386', name: 'นายวรยุทธ เนตรจ๋อย', posName: 'วิศวกรการออกแบบ', role: 'USER' },
    { code: 'MC-EN0439', name: 'นายศิวดล นนทะภา', posName: 'ผู้ช่วยวิศวกรการออกแบบ', role: 'USER' },
    { code: 'MC-EN0470', name: 'นายทรงศักดิ์ จันทร์คำลา', posName: 'วิศวกรการออกแบบ', role: 'USER' },
    { code: 'MC-EN0481', name: 'นายสมพล สุขบรรเทิง', posName: 'วิศวกรการออกแบบ', role: 'USER' },
  ...
  for (const emp of employeesData) {
    ...
    if (positionFound) {
      ...
      // 3.2 สร้างบัญชีผู้ใช้งาน (User) ผูกกับพนักงานและสิทธิ์แบบพึ่งพาความสัมพันธ์ (Relation Connect)
