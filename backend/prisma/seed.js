 ...
 */
app.get('/api/departments', async (req, res) => { // 👈 ต้องเป็นคำว่า /api/departments เท่านั้น
  try {
...
/**
 * 2. GET /api/employees
 * สำหรับดึงรายชื่อพนักงาน โดยเปลี่ยนมากรองด้วย departmentId ผ่านความสัมพันธ์ของตาราง Position
 */
...
// ✅ สั่งเปิดการทำงานของเซิร์ฟเวอร์ที่ Port 3000
app.listen(PORT, () => {
...
// ==========================================
// ส่วนของฟังก์ชัน Seed ข้อมูลตั้งต้น
// ==========================================
async function main() {
  ...
  for (const emp of employeesData) {
    // ✅ แก้ไขตรงนี้: เปลี่ยนมาใช้ === เพื่อให้ชื่อแผนกต้องตรงกันเป๊ะ ๆ ไม่สับสนระหว่าง "วิศวกรรมการ" กับ "วิศวกรรมการขาย"
    const deptFound = createdDepts.find(d => d.departmentName === emp.posName);
    
    if (deptFound) {
      const positionFound = createdPositions.find(p => p.departmentId === deptFound.id);

      if (positionFound) {
        const newEmployee = await prisma.employee.upsert({
          where: { employeeCode: emp.code },
          update: { fullName: emp.name, positionId: positionFound.id },
          create: { employeeCode: emp.code, fullName: emp.name, positionId: positionFound.id }
        });

        await prisma.user.create({
          data: { employeeId: newEmployee.id, roles: emp.role, active: true }
        });
        successCount++;
      }
    } else {
      console.log(`⚠️ ไม่พบแผนกที่ตรงตัวสำหรับพนักงาน: ${emp.name} (${emp.posName})`);
    }
  }
  console.log(`✅ สร้างพนักงานและสิทธิ์ผู้ใช้สำเร็จ ${successCount} บัญชี`);
  console.log('🎉 Seed ข้อมูลและตั้งตารางเชื่อมโยงความสัมพันธ์เสร็จสิ้น!');
}
